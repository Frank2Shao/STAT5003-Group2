...
The processed labelled dataset contains `r nrow(analysis_data)` services and `r ncol(analysis_data)` variables, including two audit identifiers and the outcome. The remaining fields cover service type and capacity, location, service offerings, operating patterns, provider age, and provider scale. A complete variable dictionary is generated as `data/processed/acecqa_variable_dictionary.csv`.

# Exploratory Data Analysis (EDA)

Categorical, numerical, and operating-pattern EDA are added by the assigned members using the common processed dataset generated above.
...
Examining the distributions of categorical variables indicates some imbalance in the variables:
...
2. **Imbalanced:** `provider_state` and `state` show that most services are located in NSW, VIC, and QLD. A trained model may underperform for new services with locations outside of NSW, VIC, and QLD.

**Rating Tier vs. Categorical Predictor**

Computing Cramer's V for each predictor to the target variable, the following variables showed strongest association: `preschool_standalone` (V=0.28), `oshc_after_school` (V=0.22), `oshc_before_school` (V=0.21), `oshc_vacation_care` (V=0.19), `operating_pattern` (V=0.16), `state` (V=0.14), and `preschool_part_school` (V=0.14). 
...
Additionally, some categorical variables have very small Cramer's V value, indicating them as weak candidates for usefulness during training. Of these, `temporarily_closed` low values might be explained by its imbalanced value distribution of approximately 100% No.
